...
async function sendNotification(email, subject, text) {
    if (!process.env.EMAIL_USER) return console.log(`[EMAIL MOCK] To: ${email} | ${subject}`);
    try { await transporter.sendMail({ from: 'RipeRide Security', to: email, subject, text }); } catch (e) {}
}
